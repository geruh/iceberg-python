...
class RestContentFile(IcebergBaseModel):
    ...
    partition: dict[str, Any] | None = Field(default=None) # Migrate to use Array
    ...
    def to_data_file(
        ...
        *,
        ...
        if self.partition and partition_spec.fields:
            # Partition is sent as {field_id: value} dict but we will handle arrays here once merged in java
...
class RestFileScanTask(IcebergBaseModel):
    ...
    def to_file_scan_task(
        ...
        *,
        ...
        # TODO: parse to evaluate that there are no equality deletes until DFI gets merged
        return FileScanTask(
            data_file=data_file,
...
class PlanTableScanRequest(IcebergBaseModel):
    ...
    filter: BooleanExpression | None = Field(default=None) # AlwaysTrue
